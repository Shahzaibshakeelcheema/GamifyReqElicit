...
const navigate=useNavigate();  
  ...
  const update=  async()=>{ 
    ...
    console.log(id)
    const response = await fetch(`http://localhost:4000/updaterequirement/${id}`, {
     method: 'PUT', 
      headers: {
       'Content-Type': 'application/json',
       'authtoken':localStorage.getItem('token')
...
<label htmlFor="edescription" className="form-label"  >Comments about updating requirement by REQ Engineer </label>
  <textarea type="text" className="form-control" id="edescription" name="edescription" rows="3" value={upreq?.Comment} ></textarea>
